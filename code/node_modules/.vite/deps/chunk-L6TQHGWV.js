...
function Mi(t2, e) {
  return void 0 === e && (e = 4), Math.ceil(t2 / e) * e - t2;
}
function Pi(t2) {
  ...
  const r = s._nextUint32(), o = s._nextUint32(), l2 = s._nextUint32(), f = s._nextUint32(), h = s._nextUint64(), U = s._nextUint64(), c = new Si(t2, Oi.length + i, 3 * a * 8, true);
  ...
  if (U <= 0) return n2;
  const x2 = new Si(t2, h, U, true), u2 = x2._nextUint16(), b = x2._nextUint16(), d = x2._nextUint32(), w = x2._nextUint32(), m = x2._nextUint32(), D = x2._nextUint32(), B2 = [];
  ...
  const L = h + x2._offset, v = L + d, A2 = v + w, k = A2 + m, V = new Uint8Array(t2.buffer, t2.byteOffset + L, d), I2 = new Uint8Array(t2.buffer, t2.byteOffset + v, w), S = new Uint8Array(t2.buffer, t2.byteOffset + A2, m), F = new Uint8Array(t2.buffer, t2.byteOffset + k, D);
...
function Wi(t2, e) {
  ...
  if (t2.globalData) {
    ...
    n2 = Ci([e2, t2.globalData.endpointsData, t2.globalData.selectorsData, t2.globalData.tablesData, t2.globalData.extendedData]);
  }
  const i = [];
  let s = t2.keyValue;
  e.keepWriter || (s = { ...t2.keyValue, KTXwriter: "KTX-Parse v0.7.1" });
  for (const t3 in s) {
    const e2 = s[t3], n3 = Ei(t3), a2 = "string" == typeof e2 ? Ci([Ei(e2), Fi]) : e2, r2 = n3.byteLength + 1 + a2.byteLength, o2 = Mi(r2, 4);
    i.push(Ci([new Uint32Array([r2]), n3, Fi, a2, new Uint8Array(o2).fill(0)]));
  }
  const a = Ci(i);
  ...
  for (let t3 = 0; t3 < r.samples.length; t3++) {
    ...
    if (e2.channelID) throw new Error("channelID has been renamed to channelType.");
  ...
  const h = Oi.length + 68 + 3 * t2.levels.length * 8, U = h + o.byteLength;
  let c = n2.byteLength > 0 ? U + a.byteLength : 0;
  c % 8 && (c += 8 - c % 8);
  const _ = [], p = new DataView(new ArrayBuffer(3 * t2.levels.length * 8)), g3 = new Uint32Array(t2.levels.length);
  let y = 0;
  0 === t2.supercompressionScheme && (y = function(t3, e2) {
    const n3 = Math.max(t3, 4), i2 = Math.min(t3, 4);
    let s2 = n3;
    for (; s2 % i2 != 0; ) s2 += n3;
    return s2;
  }(function(t3) {
    return t3.levels[0].levelData.byteLength / function(t4, e2) {
      let n3 = 1;
      const i2 = [t4.pixelWidth, t4.pixelHeight, t4.pixelDepth], s2 = function(t5) {
        const [e3, n4, i3] = t5.dataFormatDescriptor[0].texelBlockDimension;
        return [e3 + 1, n4 + 1, i3 + 1];
      }(t4);
      for (let t5 = 0; t5 < 3; t5++) if (i2[t5] > 0) {
        const e3 = Math.ceil(Math.floor(i2[t5] * Math.pow(2, -0)) / s2[t5]);
        n3 *= Math.max(1, e3);
      }
      return t4.layerCount > 0 && (n3 *= t4.layerCount), t4.faceCount > 0 && (n3 *= t4.faceCount), n3;
    }(t3);
  }(t2)));
  let x2 = (c || U + a.byteLength) + n2.byteLength;
  for (let e2 = t2.levels.length - 1; e2 >= 0; e2--) {
    if (x2 % y) {
      const t3 = Mi(x2, y);
      _.push(new Uint8Array(t3)), x2 += t3;
    }
    const n3 = t2.levels[e2];
    _.push(n3.levelData), g3[e2] = x2, x2 += n3.levelData.byteLength;
  }
  for (let e2 = 0; e2 < t2.levels.length; e2++) {
    const n3 = t2.levels[e2];
    p.setBigUint64(24 * e2 + 0, BigInt(g3[e2]), true), p.setBigUint64(24 * e2 + 8, BigInt(n3.levelData.byteLength), true), p.setBigUint64(24 * e2 + 16, BigInt(n3.uncompressedByteLength), true);
  }
  const u2 = new ArrayBuffer(68), b = new DataView(u2);
  return b.setUint32(0, t2.vkFormat, true), b.setUint32(4, t2.typeSize, true), b.setUint32(8, t2.pixelWidth, true), b.setUint32(12, t2.pixelHeight, true), b.setUint32(16, t2.pixelDepth, true), b.setUint32(20, t2.layerCount, true), b.setUint32(24, t2.faceCount, true), b.setUint32(28, t2.levels.length, true), b.setUint32(32, t2.supercompressionScheme, true), b.setUint32(36, h, true), b.setUint32(40, o.byteLength, true), b.setUint32(44, U, true), b.setUint32(48, a.byteLength, true), b.setBigUint64(52, BigInt(n2.byteLength > 0 ? c : 0), true), b.setBigUint64(60, BigInt(n2.byteLength), true), new Uint8Array(Ci([new Uint8Array(Oi).buffer, u2, p.buffer, o, a, c > 0 ? new ArrayBuffer(c - (U + a.byteLength)) : new ArrayBuffer(0), n2, ..._]));
}
...
//# sourceMappingURL=chunk-L6TQHGWV.js.map
